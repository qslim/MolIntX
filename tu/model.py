...
class NetL(torch.nn.Module):
    def __init__(self,
        ...
        self.convs = torch.nn.ModuleList()
        for i in range(config.layers):
            self.convs.append(ConvL(hidden_size=config.hidden,
                                    dropout_rate=config.dropout,
                                    nonlinear=config.nonlinear,
                                    batch_norm=batch_norm,
                                    num_basis=num_basis))
...
class ConvL(nn.Module):
    def __init__(self, hidden_size, dropout_rate, nonlinear, batch_norm, num_basis):
        ...
        self.ffn_dropout = nn.Dropout(dropout_rate)
        self.filter_drop = nn.Dropout(dropout_rate)

    def forward(self, graph, x_feat, bases):
        with graph.local_scope():
            graph.ndata['x'] = self.pre_ffn(x_feat)
            graph.edata['v'] = self.filter_drop(self.filter_encoder(bases))
            graph.update_all(fn.u_mul_e('x', 'v', '_aggr_e'), fn.sum('_aggr_e', 'aggr_e'))
            y = graph.ndata['aggr_e']
            y = self.preffn_dropout(y)
            x = x_feat + y
