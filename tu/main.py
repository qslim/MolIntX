...
def run_given_fold(num_feature,
    ...
    if config.hyperparams.get('model', 'Net') == 'NetL':
        model = NetL(num_feature, num_classes, num_basis, config=config.architecture).to(device)
    else:
...
def run_model(config):
    ...
    algo_hp = str(config.commit_id[0:7]) + '_' + str(os.environ.get('CUDA_VISIBLE_DEVICES')) \
    ...
    for i in tqdm(range(len(dataset)), desc="Pre-process"):
        ...
        dataset.graph_lists[i] = build_local_encoding(g, config.preprocess)
    ...
    if config.hyperparams.get('num_folds', 10) > 1:
        ts_fk_algo_hp = str(config.time_stamp) + '/fk' + str(config.hyperparams.get('num_folds', 10)) + '/' + algo_hp
        for i in range(1, config.hyperparams.epochs + 1):
